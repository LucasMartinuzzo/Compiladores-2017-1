...
void ypush(char yytext_push[SYMBOL_MAX_SIZE]){
...
func_decl:
   func_header func_body   { $$ = new_subtree(FUNC_DECL_NODE,2,$1,$2); set_ast_func(st,cur_func,$$);}
;
...
int main() {
    ...
    if(!dot){
        ...
        if (ret == 0) {
            ...
            int main_index = lookup_func(st,"main");
            AST *main_ast = get_ast_func(st,main_index);
            run_ast(main_ast);
        }
    }
    else
